...
// Create a new report
export async function createReport(userId: number, address: string, longitude: number, latitude: number, wasteType: string, amount: string, imageUrl: string, verificationResult: string, status: string,
    collectorId: number,
) {
    try {
...
export async function getReportsByUserId(userId: number) {
    try {
        const reports = await db.select().from(Reports).where(eq(Reports.userId, userId));
        return reports;
    } catch (error) {
...
export async function getReportsByStatus(status: string) {
    try {
        const reports = await db.select().from(Reports).where(eq(Reports.status, status));
        return reports;
    } catch (error) {
...
export async function getReportsCoordinatesByReportId(reportId: number) {
    try {
        const reportCoordinates = await db.select({
...
export async function getRewardsByUserId(rewardId: number) {
    try {
        const rewards = await db.select().from(Rewards).where(eq(Rewards.id, rewardId));
        return rewards;
    } catch (error) {
        console.log("Error reading reward: " + error);
        return null;
...
export async function getCollectedWasteByReportId(reportId: number) {
    try {
        const collectedWaste = await db.select().from(CollectedWastes).where(eq(CollectedWastes.reportId, reportId));
        return collectedWaste;
    } catch (error) {
        console.log("Error reading collected waste: " + error);
        return null;
...
export async function getTransactionsByUserId(userId: number) {
    try {
        const transaction = await db.select().from(Transactions).where(eq(Transactions.userId, userId));
        return transaction;
    } catch (error) {
        console.error("Error reading transaction: " + error);
        return null;
...
export async function getUnreadNotifications() {
    try {
        const notifications = await db.select().from(Notifications).where(eq(Notifications.isRead, false));
        return notifications;
    } catch (error) {
...
// Update report
export async function updateReport(reportId: number, userId: number, address: string, latitude: number, longitude: number, wasteType: string, amount: string, imageUrl: string, verificationResult: string, status: string) {
    try {
        await db.update(Reports).set({ userId: userId, address: address, latitude: latitude, longitude: longitude, wasteType: wasteType, amount: amount, imageUrl: imageUrl, verificationResult: verificationResult, status: status }).where(eq(Reports.id, reportId));
        console.log('Report updated!');
    } catch (error) {
        console.log("Error updating report: " + error);
    }
...
export async function updateTransactionStatus(transactionId: number, status: string) {
    try {
        await db.update(Transactions).set({ status: status }).where(eq(Transactions.id, transactionId));
...
export async function updateNotificationStatus(notificationId: number, isRead: boolean) {
    try {
        await db.update(Notifications).set({ isRead: isRead }).where(eq(Notifications.id, notificationId));
...
export async function deleteCollectedWasteByReportId(reportId: number) {
    try {
        await db.delete(CollectedWastes).where(eq(CollectedWastes.reportId, reportId));
        console.log('Collected waste deleted!');
    }
    catch (error) {
        console.log("Error deleting collected waste: " + error);
